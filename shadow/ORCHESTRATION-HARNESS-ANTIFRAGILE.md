...
| `scripts/integrity.sh` | `scripts/check.sh` ou chamada manual | stdout + exit code | humano/agent em T3+ | bloquear drift documental/contratual | `tools/integrity.sh` e auditorias de contexto do OLMO | exige valores (V1-V7), EV-B5, hook targets, cluster contract e ausencia de writes externos | remover checks que nao detectarem regressao real em 30 dias |
