...
| 2026-04-26 | sota-research-gate | Pedido do usuario: migrar de PowerShell/Windows-first para Ubuntu/WSL rapido e manter compatibilidade para merge futuro | `AGENTS.md`, `README.md`, `CLAUDE.md`, `TREE.md`, `shadow/FOUNDATION.md`, `shadow/SOTA-DECISIONS.md`, `shadow/HYGIENE.md`, `shadow/HANDOFF.md` | Ambiente confirmado: branch `codex/ubuntu-runtime-prometeus`, Ubuntu 24.04.4 LTS em WSL2, `bash`, `pwsh` e repo em `/home/...`; docs Microsoft sustentam filesystem WSL para performance. | Rodar harness local em `pwsh`; antes do merge, manter Windows como compatibilidade e checar workflow nos dois sistemas |
| 2026-04-26 | privacy-guard | `EV-B3` e risco critico `R-PHI` aberto | `shadow/DATA-CLASSIFICATION.md`, `shadow/PHI-CHECKLIST.md`, `shadow/THREAT-MODEL.md`, `shadow/INCIDENT-LOG.md`, `scripts/check.ps1`, `internal/evolution/risk-register.json` | HHS sustentou minimo necessario; NIST SP 800-61 Rev. 3 sustentou incidente como gestao de risco. O controle ficou pequeno e exigido pelo harness. | Usar checklist antes de qualquer fluxo de saude/dado pessoal; manter incidente sem conteudo sensivel |
| 2026-04-26 | bash-harness | Pedido do usuario: tirar PowerShell e ficar em WSL2 | `scripts/check.sh`, `scripts/evolve.sh`, `.github/workflows/self-evolution.yml`, `AGENTS.md`, `README.md`, `shadow/FOUNDATION.md`, `shadow/SOTA-DECISIONS.md` | `./scripts/check.sh` e `./scripts/evolve.sh check` passaram em Ubuntu/WSL sem `pwsh`; `.ps1` ficou legado temporario, nao gate principal. | Comitar e rodar `./scripts/check.sh --strict`; proximo passo e remover ou portar scripts `.ps1` restantes quando Bash cobrir os casos especificos |
